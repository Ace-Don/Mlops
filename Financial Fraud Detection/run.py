...
# Set MLflow tracking URI globally (Docker MLflow on port 5000)
...
def main(
    ...
    if training_pipeline:
        ...
        # Run the pipelines for all selected models
        for model_name in ["lr", "xgb", "catboost", "lightgbm"]:
            pipeline_args = {}
            if no_cache:
                pipeline_args["enable_cache"] = False
            pipeline_args["config_path"] = os.path.join(config_folder, f"training_{model_name}.yaml")
            training.with_options(**pipeline_args)(**run_args_train)
            logger.info(f"Training pipeline with {model_name.upper()} finished successfully!\n\n")
