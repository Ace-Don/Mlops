...
def model_promoter(accuracy: float, stage: str = "production") -> bool:
    ...
    mlflow.start_run(run_name="model_promotion_decision")
    
    # Log promotion parameters
    mlflow.log_param("stage", stage)
    mlflow.log_param("accuracy", accuracy)
    mlflow.log_param("accuracy_threshold", 0.8)
    ...
    if accuracy < 0.8:
        ...
        mlflow.log_param("promotion_status", "rejected_low_accuracy")
    else:
        ...
        try:
            ...
            prod_accuracy = (
                stage_model.get_artifact("trained_model")
            ...
            if float(accuracy) > float(prod_accuracy):
                ...
                mlflow.log_param("promotion_status", "promoted_better_accuracy")
                mlflow.log_metric("previous_accuracy", prod_accuracy)
            else:
                mlflow.log_param("promotion_status", "rejected_worse_accuracy")
                mlflow.log_metric("previous_accuracy", prod_accuracy)
        except KeyError:
            # If no such model exists, current one is promoted
            is_promoted = True
            current_model.set_stage(stage, force=True)
            mlflow.log_param("promotion_status", "promoted_first_model")
