...
def data_splitter(
    ...
    mlflow.start_run(run_name="data_split_train_test")
    
    # Log split parameters and dataset shapes
    ...
    # Log split results as metrics (actual numbers, not configuration)
    mlflow.log_metric("train_rows", len(dataset_trn))
    mlflow.log_metric("test_rows", len(dataset_tst))
    mlflow.log_metric("train_percentage", (len(dataset_trn) / len(dataset)) * 100)
